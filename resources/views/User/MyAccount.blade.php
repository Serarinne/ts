...
<head>
	...
	<meta name="twitter:title" content="My Account | WaifuWall">
...
<body>
	...
	<section name="PostData" class="mx-3 my-3">
		<div class="bg-gray-800 intro-y box mt-5 px-5 pt-5 rounded-md">
			...
			<ul role="tablist" class="w-full flex flex-col my-2 justify-center text-center sm:flex-row lg:justify-start">
				<li role="presentation" class="focus-visible:outline-none">
					<span role="tab" class="block bg-gray-900 text-white font-medium py-2 px-5 mb-2 mx-2" aria-current="page">Account</span>
				</li>
				<li role="presentation" class="focus-visible:outline-none">
		...
		<div class="container mx-auto p-8">
			<h3 class="font-medium text-gray-900 text-left px-6">Statistics</h3>
			@if(session()->has('msg'))
			...
			<table class="min-w-full divide-y divide-gray-200">
				<tbody>
					<tr class="bg-white">
						...
						<td class="px-6 py-4 whitespace-nowrap">
							<div class="text-sm text-gray-500">{{ $UserData->id }}</div>
						</td>
					</tr>
					<tr class="bg-gray-50">
						<td class="px-6 py-4 whitespace-nowrap">
						...
						<td class="px-6 py-4 whitespace-nowrap">
							<div class="text-sm text-gray-500">@if($UserData->role == 0)Admin @elseif($UserData->role == 1)Member @elseif($UserData->role == 2)Moderator @endif</div>
						</td>
					</tr>
					<tr class="bg-white">
						<td class="px-6 py-4 whitespace-nowrap">
							<div class="text-sm font-medium text-gray-900">Joined</div>
						</td>
						<td class="px-6 py-4 whitespace-nowrap">
							<div class="text-sm text-gray-500">{{ date("Y-m-d", strtotime($UserData->date)) }}</div>
						</td>
					</tr>
					<tr class="bg-gray-50">
						<td class="px-6 py-4 whitespace-nowrap">
					...
					<tr class="bg-white">
						<td class="px-6 py-4 whitespace-nowrap">
							<div class="text-sm font-medium text-gray-900">Full Name</div>
						</td>
						<td class="px-6 py-4 whitespace-nowrap">
					...
					<tr class="bg-white">
						...
						<td class="px-6 py-4 whitespace-nowrap">
							<div class="text-sm text-gray-500">@if(is_null($UserData->validate_token))Trusted User @elseif($UserData->validate_token == "")Verified User @else Unverified User @endif @if(!$UserData->validate_token == NULL || !$UserData->validate_token == "")(<a href="{{ str_replace(request()->getHost(), $ServerDomain, url('/resend-verification')) }}" class="text-blue-500">Resend verification</a>) @endif</div>
						</td>
					</tr>
					<tr class="bg-gray-50">
						<td class="px-6 py-4 whitespace-nowrap">
							<div class="text-sm font-medium text-gray-900">Uploaded wallpaper</div>
						</td>
						<td class="px-6 py-4 whitespace-nowrap">
							<div class="text-sm text-gray-500">{{ $WallpaperData }}</div>
						</td>
					</tr>
					<tr class="bg-white">
						<td class="px-6 py-4 whitespace-nowrap">
							<div class="text-sm font-medium text-gray-900">Favorite wallpaper</div>
						</td>
						<td class="px-6 py-4 whitespace-nowrap">
							<div class="text-sm text-gray-500">{{ $FavoriteData }}</div>
						</td>
					</tr>
					<tr class="bg-gray-50">
						<td class="px-6 py-4 whitespace-nowrap">
							<div class="text-sm font-medium text-gray-900">Posted article</div>
						</td>
						<td class="px-6 py-4 whitespace-nowrap">
							<div class="text-sm text-gray-500">{{ $BlogData }}</div>
						</td>
					</tr>
				</tbody>
			</table>
